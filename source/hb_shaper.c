...
hb_shape_result_t hb_shape_text(const char *text)
{
    ...
    // To Align text based on display
//    result.ascender    = face->size->metrics.ascender >> 6;
//    result.descender   = -(face->size->metrics.descender >> 6);
//    result.line_height = face->size->metrics.height >> 6;
